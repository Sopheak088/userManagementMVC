...
{
    public class IdentityGroupManager
    {
        private ApplicationGroupStore _groupStore;
        private UserCenterDbContext _db;
        private IdentityUserManager _userManager;
        private IdentityRoleManager _roleManager;

        public IdentityGroupManager()
        {
            _db = HttpContext.Current
                .GetOwinContext().Get<UserCenterDbContext>();
            _userManager = HttpContext.Current
                .GetOwinContext().GetUserManager<IdentityUserManager>();
            _roleManager = HttpContext.Current
                .GetOwinContext().Get<IdentityRoleManager>();
        ...
        public IdentityResult SetGroupRoleNames(string groupId, params string[] roleNames)
        {
        ...
        public async Task<IdentityResult> SetGroupRoleNamesAsync(string groupId, params string[] roleNames)
        {
            // Clear all the roles associated with this group:
        ...
        public IdentityResult SetGroupRoleIds(string groupId, params string[] roleIds)
        {
            // Clear all the roles associated with this group:
            var thisGroup = this.FindById(groupId);
            thisGroup.ApplicationRoles.Clear();
        ...
        public async Task<IdentityResult> SetGroupRoleIdsAsync(string groupId, params string[] roleIds)
        {
            // Clear all the roles associated with this group:
            var thisGroup = await this.FindByIdAsync(groupId);
            thisGroup.ApplicationRoles.Clear();
        ...
        public IEnumerable<ApplicationGroup> GetUserGroups(string userId)
        {
            ...
            var userGroups = (from g in this.Groups
                              where g.ApplicationUsers.Any(u => u.ApplicationUserId == userId)
                              select g).Include(m => m.ApplicationProject).ToList();
            return userGroups;
        }

        public async Task<IEnumerable<ApplicationRole>> GetGroupRolesAsync(string groupId)
        {
            var grp = await _db.AspNetGroups.FirstOrDefaultAsync(g => g.Id == groupId);
        ...
        {
            var grp = _db.AspNetGroups.Include(m => m.ApplicationRoles).Include(m => m.ApplicationUsers).FirstOrDefault(g => g.Id == groupId);
